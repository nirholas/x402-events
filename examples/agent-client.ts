/**
 * x402-events — agent client example.
 *
...
async function main(): Promise<void> {
  ...
  const unpaid = await fetch(`${BASE_URL}/search?keyword=jazz&city=Chicago&countryCode=US&size=2`);
  ...
  {
    const res = await payFetch(`${BASE_URL}/search?keyword=jazz&city=Chicago&countryCode=US&size=2`);
    if (!res.ok) {
      console.error(`GET /search failed: ${res.status}`, await res.text());
      process.exit(1);
    }
    console.log("=== GET /search ($0.003) ===");
  ...
  {
    const res = await payFetch(`${BASE_URL}/onsale-check/FIX1RW2TPK1`);
    if (!res.ok) {
      console.error(`GET /onsale-check/:eventId failed: ${res.status}`, await res.text());
      process.exit(1);
    }
    console.log("=== GET /onsale-check/:eventId ($0.002) ===");
    console.log(JSON.stringify(await res.json(), null, 2));
